...
module master_control(
    ...
    always @ (posedge clk_5mhz) begin
        if ( set_time | ~count_enable) begin count_or_cook <= 1; end 
        else if ( ~set_time | count_enable) begin count_or_cook <= 0; end 
        else begin count_or_cook <= count_or_cook; end   
    end
    ...
    always @(posedge clk_500hz or posedge rst) begin
       if (rst) begin count <= 3'b000; seg <= 0; an <= 8'b11111111; end 
       else begin
           case(count)
                3'b000:begin seg <= seg_out1; an <= 8'b11111110; count <= count + 1; end
                3'b001:begin seg <= seg_out2; an <= 8'b11111101; count <= count + 1; end
                3'b010:begin seg <= seg_out3; an <= 8'b11111011; count <= count + 1; end
                3'b011:begin seg <= seg_out4; an <= 8'b11110111; count <= count + 1; end
                3'b100:begin an <= 8'b11111111; count <= count + 1; end
                3'b101:begin an <= 8'b11111111; count <= count + 1; end
                3'b110:begin an <= 8'b11111111; count <= count + 1;end
                3'b111:begin an <= 8'b11111111; count <= 3'b000; end
                default:begin count <= 3'b000; end
           endcase
       end
    end 
